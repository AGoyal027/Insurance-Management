...
('839aaa330', 'Rajiv', '1997-06-26', 'Indira Bazar, MI Road', 302015, 'jaipur', 8929788295);
...
INSERT INTO `customer` (`Customer_Num`, `First_Name`, `Last_Name`, `Gender`, `DOB`, `Address`, `Pincode`, `Contact_Number`, `Mother_Name`, `Mother_Status`, `Father_Name`, `Father_Status`, `Marital_status`) VALUES
(10004, 'Devam', 'Sheth', 'M', '2000-10-02', '21/694, Satyam Apartment, Refinery Road, Gorwa', 390016, 7016636683, 'Harsha Sheth', 'A', 'Sanjay Sheth', 'A', 'S'),
(10006, 'Ayush', 'Goyal', 'M', '2001-12-01', 'shri goyal electric works, roopangarh road, madanganj - kishangarh', 305801, 9413411827, 'Mrs Goyal', 'A', 'Mr Goyal', 'A', 'S');
...
(123564789, 10004, '234abc231', '2018-10-02', 'Jeevan Labh', 35000, 5, 10),
(284049583, 10004, '234abc231', '2007-06-20', 'Jeevan Lakshya', 450000, 35, 80);
...
/*CREATION OF TRIGGER*/
CREATE TRIGGER `insertLog` AFTER INSERT ON `agent` FOR EACH ROW INSERT INTO logs VALUES(null, NEW.agent_code, 'Inserted', NOW());

CREATE TRIGGER `updateLog` AFTER UPDATE ON `agent` FOR EACH ROW INSERT INTO logs VALUES(null, NEW.agent_code, 'Updated', NOW());

CREATE TRIGGER `deleteLog` AFTER UPDATE ON `agent` FOR EACH ROW INSERT INTO logs VALUES(null, OLD.agent_code, 'Deleted', NOW());

/*CREATION OF STORED PROCEDURE*/
CREATE PROCEDURE `getAgents`() NOT DETERMINISTIC CONTAINS SQL SQL SECURITY DEFINER SELECT Agent_code, Address, Contact_Num FROM agent

CREATE PROCEDURE `getCustomers`() NOT DETERMINISTIC CONTAINS SQL SQL SECURITY DEFINER SELECT Customer_Num, First_Name, Gender, Address, Contact_Number FROM customer
